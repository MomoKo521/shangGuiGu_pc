  ...
  <div class="trade-container">
    ...
    <div class="content">
      ...
      <div class="detail">
        ...
        >
          <li>
            <img :src="item.imgUrl" style="height: 100px" alt="" />
          </li>
          <li>
            <p>
              {{ item.skuName }}
...
export default {
  ...
  methods: {
    ...
    changeaddress(idnex) {
      ...
      this.addressInfo[idnex].isDefault = 1;
    },
    // 点击提交订单
    async submitOrder() {
      //准备提交订单需要的请求参数
      ...
      //data
      let query = {
        consignee: this.userDefaultAddress.consignee, //收获人名字
        consigneeTel: this.userDefaultAddress.phoneNum, //收货人电话
        deliveryAddress: this.userDefaultAddress.fullAddress, //收获地址
        paymentWay: "ONLINE", //支付方式
        orderComment: this.message, //留言
        orderDetailList: this.orderInfo.detailArrayList, //商品清单(要购买的购物商品)
      };
      // 发送提交订单请求
      // 因为这次 换了一种axios请求 使用全局事件总线全局挂载了axios封装函数文件  所以能直接访问到封装好的aixos函数
      let refult = await this.$API.reqSubmitOrder(tradeNo, query);
      console.log(refult);
      if (refult.code == 200) {
        this.$router.push(`/pay?tradeNo=${refult.data}`);
        // 成功
      } else {
  ...
  computed: {
    ...
    // 用户选中的当前地址
    userDefaultAddress() {
      // find:查找数组当中符合条件的元素返回，并为最终结果 \
      //addressInfo是异步数据 一开始拿是空数组
