    ...
    <section class="con">
      ...
      <div class="mainCon">
        ...
        <div class="previewWrap">
          <!--放大镜效果-->
          <Zoom :bigimgURL="skuInfo.skuDefaultImg" />
          <!-- 小图列表 -->
          <ImageList
            :skuImage="skuInfo.skuImageList"
            @updeteBigImg="updeteBigImg"
          />
        </div>
        <!-- 右侧选择区域布局 -->
        <div class="InfoWrap">
          ...
          <div class="choose">
            <div class="chooseArea">
              ...
              <dl v-for="item in spuSaleAttrList" :key="item.id">
                ...
                <dd
                  changepirce="0"
                  :class="{ active: itemson.isChecked == 1 }"
                  v-for="itemson in item.spuSaleAttrValueList"
                  :key="itemson.id"
                  @click="cangeActive(itemson, item.spuSaleAttrValueList)"
                >
                  {{ itemson.saleAttrValueName }}
                </dd>
              </dl>
            </div>
            <div class="cartWrap">
              <div class="controls">
                ...
                <a
                  href="javascript:"
                  class="mins"
                  @click="number > 1 ? number-- : ''"
                  >-</a
                >
              </div>
              <div class="add">
                <a href="javascript:" @click="addGoodCart">加入购物车</a>
              </div>
            </div>
          </div>
        </div>
...
export default {
  ...
  mounted() {
    // 发送axios请求获取数据
    this.$store.dispatch("getGoodInfo", this.$route.params.sukid);
  },
  computed: {
  ...
  methods: {
    ...
    // 自定义事件  获取子组件点击的轮播图图片
    updeteBigImg(bigimg) {
      //  console.log(this.skuInfo.skuDefaultImg);
      this.skuInfo.skuDefaultImg = bigimg;
    },
    // 商品数量发生改变
    changeNumber() {
      // 为空就等于1
    ...
    // 点击加入购物车按钮发送商品加入购物车请求
    async addGoodCart() {
      try {
        // 成功
        await this.$store.dispatch("addGoodCart", {
          skuId: this.$route.params.sukid,
          skuNum: this.number,
        });
        // 商品详细信息用户会话存储到浏览器
        // 注意会话存储不能存储对象类型，所以我们得把对象转为json格式
